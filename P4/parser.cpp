...
vector<identValues> identifierValues;
...
// Determine if an identifer has been declared previously
bool verify(token token) {
    for (int i = 0; i < identifiers.size(); i++) {
        if (identifiers.at(i).name == token.name) {
            return true;
        }
    }
...
// Generate error for reused identifier
node_t* reuseError(node_t* node) {
    node->status = "ERROR";
    node->lineNum = tk.line;
    node->tkErrorName = tk.name;
    node->tkErrorType = "REUSED_IDENT";

    cout << "Parser Error: " << node->tkErrorType << ", token: " 
                 << node->tkErrorName << ", Line: " << node->lineNum << endl;
    return node;
}

// Generate error for undeclared undeclared identifer
node_t* undeclaredError(node_t* node) {
    node->status = "ERROR";
    node->lineNum = tk.line;
    node->tkErrorName = tk.name;
    node->tkErrorType = "UNDECLARED_IDENT";
...
node_t* V() {
    ...
    if (tk.tkID == "VAR_TK") {
        ...
        if (tk.tkID == "IDENT_TK") {
            node->tk2 = tk;
            if (verify(tk)) return reuseError(node);
            identifiers.push_back(tk);
            tk = scanner();
...
node_t* A1() {
    ...
    if (tk.tkID == "IDENT_TK") {
        node->tk1 = tk;
        if (!verify(tk)) return undeclaredError(node);
        tk = scanner();
        return node;
    }
